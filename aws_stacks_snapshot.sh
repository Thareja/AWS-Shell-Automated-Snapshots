#!/bin/bash
# Filename    : aws_stacks_snapshot.sh
# Created by  : Dhiraj Thareja - dhiraj_thareja@trimble.com
# Version     : 1.1
...
LOG_FILE=/var/log/stack_snapshots_$DATE.log #update the path for your choice
FROM="your@email.com" #provide some random email id as from address
TO="your@email.com" #Update the email id which you want to receive email alerts"
REGION="us-east-1"
STACKS=/opt/stack_names


#StackName="LAMP stack-xxxxxx" #Enter Stack Name here. example: LAMP,JAVA
cat $STACKS | while read StackName
do
COUNT=`ec2-describe-tags  --region $REGION  --filter "resource-type=instance" | grep "StackName" | grep "$StackName$" | awk '{ print $3 }' | sort -u | wc -l`
 if [ $COUNT -eq 0 ]
 then
 echo "Stack : $StackName" | tee -a $LOG_FILE
 echo "No isnatnces are found with stack Name $StackName" | tee -a $LOG_FILE
 echo "Jump to next stack" | tee -a $LOG_FILE
 echo "#########################"
 else
        echo "Stack : $StackName" | tee -a $LOG_FILE
        echo "Found instances assosiated with stack $StackName" | tee -a $LOG_FILE
        for i in ` ec2-describe-tags  --region $REGION  --filter "resource-type=instance" | grep "StackName" | grep "$StackName$" | awk '{ print $3 }' | sort -u`; do
        ...
        ec2-describe-tags --region $REGION  --filter "resource-id=$i" | tee -a $LOG_FILE
                ...
                ec2-create-snapshot --description $SNAP_DESC --region $REGION $j
                        ...
                        sleep 5
                                ...
                                do
                                echo " " | tee -a $LOG_FILE
                                echo "####Adding the tag "#$tag#"####" | tee -a $LOG_FILE
